...
Node* insertBST(Node* node, int value) {
    ...
    // Duplicate values are typically ignored in a basic BST

    return node;
}

// 3. Traversal Functions (Keep your existing ones)
...
int main() {
    ...
    do {
        ...
        while (scanf("%d", &x) == 1) root = insertBST(root, x);
        while(getchar() != '\n'); 
        ...
        while (1) {
            ...
            if (choice == 1) {
                printf("Inorder: "); displayIn(root); printf("\n");
            } else if (choice == 2) {
